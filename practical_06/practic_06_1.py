...
with open(r'C:\Users\cansu\OneDrive\Desktop\PAML_2024\practical_06\mvg_outlier.pkl', 'rb') as file:
...
def plot_data(original, z_filtered, mod_z_filtered):
    ...
    x_min = min(np.min(original[:, 0]), np.min(z_filtered[:, 0]), np.min(mod_z_filtered[:, 0]))
    x_max = max(np.max(original[:, 0]), np.max(z_filtered[:, 0]), np.max(mod_z_filtered[:, 0]))
    y_min = min(np.min(original[:, 1]), np.min(z_filtered[:, 1]), np.min(mod_z_filtered[:, 1]))
    y_max = max(np.max(original[:, 1]), np.max(z_filtered[:, 1]), np.max(mod_z_filtered[:, 1]))
